                    ...
                    <div key={key} className="space-y-1.5">
                        ...
                        {isMultiline ? (
                            <div className="rounded-md border border-border/50 bg-muted/30">
                                <CodeBlock code={value as string} language={name === 'WriteToFile' ? 'text' : 'json'} />
                            </div>
                        ) : (
...
export const ToolOutput = ({
    ...
    if (errorText) {
        ...
        if (output.content && typeof output.content === 'string') {
            ...
            Output = <div className="border border-border/50 rounded-md overflow-hidden bg-muted/30"><CodeBlock code={output.content} language={lang || 'text'} /></div>;
